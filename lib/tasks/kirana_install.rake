...
namespace :kirana do
  desc "Create a Kirana Application"
...
use rake db:load_file[/absolute/path/to/sample/filename.rb]}
    ...
    desc "Bootstrap is: migrating, loading defaults and seeding (for all extensions) and load_products tasks"
    task :bootstrap  do
      require 'highline/import'
      ...
      if yes?("Do you want to create Spree admin (Yes/No)?")
              Rake::Task["kirana_admin:create"].invoke
      end
