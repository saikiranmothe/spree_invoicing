...
#Company Account Details
 ...
 # end
  ...
  pdf.table([ ["S.No","Description","Quantity","Unit Price"] ],:column_widths => [40,330,60,110], cell_style: {align: :center})
  @count = 0
  @order.line_items.each do |i|
    ...
    @item_price = i.price
    @count += 1
    pdf.table([ ["#{@count}","#{@name}","#{@quantity}","#{@item_price}"] ],:column_widths => [40,330,60,110], cell_style: {align: :center})
  end

  pdf.table([ ["","","Sub-Total","#{@order.display_item_total}".gsub(/₹/, ' ')] ],:column_widths => [40,330,60,110], cell_style: {align: :center})
  pdf.table([ ["","","VAT(5%)","#{@order.tax_total}"] ],:column_widths => [40,330,60,110], cell_style: {align: :center})
  pdf.table([ ["","","Shipping","#{@order.display_ship_total}".gsub(/₹/, ' ')] ],:column_widths => [40,330,60,110], cell_style: {align: :center})
  pdf.table([ ["","","Total Due","#{@order.display_total}".gsub(/₹/, ' ')] ],:column_widths => [40,330,60,110], cell_style: {align: :center})
  ...
  ##############################################
    ...
    pdf.text "* This is a Computarized Generated Bill & requires no Signature"
    pdf.stroke_horizontal_rule
    pdf.move_down 5
